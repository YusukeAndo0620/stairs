...
class BoardPosition extends _$BoardPosition {
  ...
  void setBoardPosition({required String boardId, required GlobalKey key}) {
    ...
    state = state.copyWith(
      boardPositionMap: targetMap,
    );
  }

  void setTaskItemPosition(
      {required String taskItemId, required GlobalKey key}) {
    if (key.currentContext == null) return;
    final position = key.currentContext!.findRenderObject() as RenderBox;
